...
def load_data(uploaded_file=None):
    """读取上传数据；如果没有上传，则读取项目根目录下的 model_sample.csv。"""
    if uploaded_file is not None:
        return pd.read_csv(uploaded_file, encoding="utf-8-sig")

    if not DEFAULT_DATA.exists():
        st.error(
            "未找到默认数据文件：model_sample.csv。\n\n"
            "请确认 model_sample.csv 与 app.py 位于同一目录，"
            "或者在左侧上传 CSV 数据后继续使用。"
        )
        st.stop()
...
def generate_markdown_report(df):
    ...
    if len(corr_cols) >= 2:
        corr_md = df[corr_cols].corr(numeric_only=True).round(3).to_markdown()
    else:
...
def generate_research_assistant_prompt(df):
    ...
    prompt = f"""
你是一名统计学与应用计量方向的研究助手。请基于下面的项目信息，帮助我生成适合论文写作和答辩准备的研究解释。

项目背景：
我正在分析空气污染与呼吸系统健康负担之间的统计关系。数据为省级年度面板数据，样本量为 {n_rows} 条观测，字段数为 {n_cols} 个。
...
要求：
语言严谨，适合统计学本科毕业论文和研究生科研训练场景，不要编造不存在的数据结果。
"""
    return prompt.strip()
...
with tab1:
    ...
    c3.metric("省份数", df_raw["省份"].nunique() if "省份" in df_raw.columns else "未识别")
    c4.metric("年份范围", f"{df_raw['年份'].min()}—{df_raw['年份'].max()}" if "年份" in df_raw.columns else "未识别")

    if "所属区域" in df_raw.columns:
        st.subheader("区域分布")
...
with tab2:
    ...
    if "年份" in df_raw.columns and trend_cols:
    ...
    if "所属区域" in df_raw.columns and region_cols:
        ...
        st.warning("当前数据缺少区域或对比变量，无法生成区域均值图。")
...
with tab4:
    ...
    if len(selected_pollutants) < 2:
        ...
        try:
            ...
            if scores.shape[1] > 1:
                d_pca["PC2"] = scores[:, 1]
                pca_rhs = ["PC1", "PC2"] + controls + ["C(province)", "C(year)"]
                vars_show = ["PC1", "PC2"]
            else:
                pca_rhs = ["PC1"] + controls + ["C(province)", "C(year)"]
                vars_show = ["PC1"]

            pca_formula = f"{y} ~ " + " + ".join(pca_rhs)
...
with tab6:
    st.subheader("报告辅助")
    st.markdown("这个模块用于把统计结果转化为论文写作提纲。它不调用任何外部服务，只生成可复制的研究报告模板。")
